...
st.set_page_config(
     page_title="Project Data science",
     page_icon="🧊",
     layout="wide",
...
if menu == 'Introduction':
    ...
    st.write(' Attributes: ID,Price: price of the cars(Target Column)Levy,Manufacturer,Model,Prod. year,Category,Leather interior,Fuel type,Engine volume,Mileage,Cylinders,Gear box type,Drive wheels,Doors,Wheel,Color,Airbags')
    st.header('Data')
    ...
    if cat == 'Manufacturer':
        ...
        sns.set_style('dark')
        x= sns.barplot(Manufacturer.index, Manufacturer, palette ='Paired')
        x.set_xticklabels(Manufacturer.index, rotation = 90)
        x.set(xlabel ='Manufacturer', ylabel = 'Values')
        st.pyplot(fig1)
    elif cat=='Model':
    ...
    if tree == 'Cars':
        ...
        distribution1 = px.treemap(data_frame=df,path=["Manufacturer","Category","Model"],values='Price')
